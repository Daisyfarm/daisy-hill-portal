...
import { Store, Tag, DollarSign, PlusCircle, User } from 'lucide-react';
...
export default function MarketplacePage() {
  ...
  const load = async () => {
    ...
    if (data && data.length > 0) {
      ...
      setListings([
        { id: 1, title: 'John Deere 8R 410 (Low Hours)', category: 'Heavy Machinery', price: 285000, seller: { username: 'DaisyFarmer' }, server: 'Server 19' },
        { id: 2, title: 'Field 22 Plot (14.5 Hectares)', category: 'Land Parcels', price: 450000, seller: { username: 'NorthPlainsCoop' }, server: 'Server 8' },
        { id: 3, title: 'Seed Potato Pallets (Bulk x10)', category: 'Supplies', price: 4200, seller: { username: 'AlpineAgro' }, server: 'Server 4' }
      ]);
    }
    setLd(false);
  };
  ...
  const handleAddListing = async (e: React.FormEvent) => {
    ...
    if (!title || isNaN(numericPrice)) return;
    ...
    if (!error) {
      await fetch(HK, {
        ...
        body: JSON.stringify({
          content: `🏷️ **MARKETPLACE LISTING PUBLISHED**\n**Seller:** ${u.username}\n**Item:** ${title} (${category})\n**Asking Price:** $${numericPrice.toLocaleString()}`
        })
      });
  ...
  if (ld) return <div style={{background:'#111827',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Accessing Marketplace Hub...</div>;
            ...
            <div key={item.id} style={{ background: '#1f2937', border: '1px solid #374151', borderRadius: '8px', padding: '22px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#9ca3af' }}>{item.server || 'Judith Plains 4X'}</span>
                </div>
                <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#fff', margin: '0 0 8px 0' }}>{item.title}</h3>
                <div style={{ fontSize: '22px', fontWeight: 'bold', color: '#4ade80', marginBottom: '12px' }}>${(item.price || 0).toLocaleString()}</div>
                <div style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '15px' }}>Seller: <strong style={{ color: '#fff' }}>{item.seller?.username || item.seller || 'Verified Operator'}</strong></div>
              </div>
              <button onClick={() => alert(`Contacted seller regarding ${item.title}!`)} style={{ background: '#374151', color: '#fff', border: '1px solid #4b5563', padding: '8px', fontWeight: 'bold', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', width: '100%' }}>
                CONTACT SELLER
              </button>
            </div>
          ))}
