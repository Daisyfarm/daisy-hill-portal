    ...
    <div style={{ 
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
          <div>
            <div style={{ color: '#34d399', fontSize: '10px', fontWeight: 900, textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '0.1em', textShadow: '0 2px 4px rgba(0,0,0,0.8)' }}>
              Regional Schedule • Town Halls & Auctions
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          {[
            { event: 'Annual Harvest Machinery Auction', date: 'Tomorrow, 10:00 AM', location: 'Central Pavilion', status: 'Registration Open' },
            { event: 'Regional Grain Growers Town Hall', date: 'Aug 2, 2:00 PM', location: 'Sector Alpha Hub', status: 'Upcoming' },
            { event: 'Agritech Innovation Showcase', date: 'Aug 10, 9:00 AM', location: 'Exhibition Center', status: 'Planning' },
          ].map((item, idx) => (
            <div key={idx} style={{ backgroundColor: 'rgba(11, 14, 20, 0.75)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: '12px', padding: '24px' }}>
              <div style={{ fontSize: '10px', fontWeight: 900, color: '#34d399', textTransform: 'uppercase', marginBottom: '8px' }}>{item.location}</div>
              <h3 style={{ fontSize: '18px', fontWeight: 900, color: '#ffffff', margin: '0 0 12px 0', textTransform: 'uppercase' }}>{item.event}</h3>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '8px' }}>
                <span style={{ color: '#a1a1aa' }}>Date:</span>
                <span style={{ color: '#ffffff', fontWeight: 900 }}>{item.date}</span>
              </div>
