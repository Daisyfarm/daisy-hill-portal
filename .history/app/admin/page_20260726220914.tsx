...
import { Cloud, ShieldAlert, Shield, BookOpen, ArrowLeft, Users, CheckCircle2, Server, Truck, TrendingUp } from 'lucide-react';
    ...
    <div style={{ background:'#090d16', minHeight:'100vh', color:'#f8fafc', fontFamily:'Inter, system-ui, sans-serif', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, background:'linear-gradient(rgba(9, 13, 22, 0.85), rgba(9, 13, 22, 0.95)), url("https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=1600") center/cover', padding:'40px' }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto', display:'flex', flexDirection:'column', gap:'30px' }}>
          ...
          {/* BANNER */}
          <div style={{ background:'rgba(17, 24, 39, 0.85)', backdropFilter:'blur(10px)', padding:'30px 40px', borderRadius:'12px', border:'1px solid #374151', boxShadow:'0 10px 25px -5px rgba(0, 0, 0, 0.5)', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'20px' }}>
            <div>
              ...
              <p style={{fontSize:'13px', color:'#94a3b8', margin:'6px 0 0 0'}}>Manage operator profiles, server infrastructure, shared assets, and economy metrics.</p>
            </div>
          ...
          {/* ADMIN MANAGEMENT CARDS SECTION */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            
            <div onClick={() => window.location.href='/server-setup'} style={{ background: 'rgba(17, 24, 39, 0.85)', backdropFilter:'blur(10px)', padding: '25px', borderRadius: '12px', border: '1px solid #374151', cursor: 'pointer', transition: 'border-color 0.2s', display:'flex', flexDirection:'column', gap:'10px', boxShadow:'0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}>
              <div style={{display:'flex', alignItems:'center', gap:'8px', color:'#22c55e'}}>
                <Server size={20} />
                <h3 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0, color: '#f8fafc' }}>G-Portal Server Setup</h3>
              </div>
              <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0, lineHeight:'1.5' }}>Configure server instance, active maps, and slot counts prior to launch.</p>
            </div>

            <div onClick={() => window.location.href='/fleet'} style={{ background: 'rgba(17, 24, 39, 0.85)', backdropFilter:'blur(10px)', padding: '25px', borderRadius: '12px', border: '1px solid #374151', cursor: 'pointer', transition: 'border-color 0.2s', display:'flex', flexDirection:'column', gap:'10px', boxShadow:'0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}>
              <div style={{display:'flex', alignItems:'center', gap:'8px', color:'#22c55e'}}>
                <Truck size={20} />
                <h3 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0, color: '#f8fafc' }}>Cooperative Fleet Garage</h3>
              </div>
              <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0, lineHeight:'1.5' }}>Inspect and manage shared machinery, maintenance, and equipment allocations.</p>
            </div>

            <div onClick={() => window.location.href='/market'} style={{ background: 'rgba(17, 24, 39, 0.85)', backdropFilter:'blur(10px)', padding: '25px', borderRadius: '12px', border: '1px solid #374151', cursor: 'pointer', transition: 'border-color 0.2s', display:'flex', flexDirection:'column', gap:'10px', boxShadow:'0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}>
              <div style={{display:'flex', alignItems:'center', gap:'8px', color:'#22c55e'}}>
                <TrendingUp size={20} />
                <h3 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0, color: '#f8fafc' }}>Commodity Market Control</h3>
              </div>
              <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0, lineHeight:'1.5' }}>Monitor crop prices, market trends, and top-yielding selling stations.</p>
            </div>

          </div>

          {msg && (
            <div style={{background:'rgba(74, 222, 128, 0.1)', border:'1px solid #4ade80', color:'#4ade80', padding:'12px 20px', borderRadius:'8px', fontSize:'13px', fontWeight:'600', display:'flex', alignItems:'center', gap:'8px'}}>
