...
export default function Accounting() {
  ...
  const load = async () => {
    try {
      ...
      const activeUser = user || (await sb.auth.getUser()).data.user;
      if (!activeUser) {
        window.location.href = '/';
        return;
      }

      const { data: profile } = await sb.from('profiles').select('*').eq('id', activeUser.id).single();
      setU(profile || { balance: 0, credit_score: 600 });

      const { data: land } = await sb.from('land_registry').select('price').eq('owner_id', activeUser.id);
      const landSum = land?.reduce((acc, curr) => acc + (curr.price || 0), 0) || 0;

      const { data: fleet } = await sb.from('fleet').select('value').eq('owner_id', activeUser.id);
      const fleetSum = fleet?.reduce((acc, curr) => acc + (curr.value || 0), 0) || 0;

      const { data: loans } = await sb.from('loans').select('amount_remaining').eq('user_id', activeUser.id).eq('status', 'active');
      const debtSum = loans?.reduce((acc, curr) => acc + (curr.amount_remaining || 0), 0) || 0;

      const { data: tx } = await sb.from('transactions').select('amount').eq('user_id', activeUser.id).ilike('description', '%Fine%');
