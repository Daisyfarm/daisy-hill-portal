...
export default function RegistrationPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Farm sector successfully registered and added to network!');
      setFarmName('');
      setOwnerName('');
      setAcreage('');
      setContactEmail('');
      ...
      console.error(err);
      // Fallback local update if network/table insert fails
  ...
  if (ld) {
    return (
      <div style={{background:'#090d16', color:'#38bdf8', height:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:'Inter, sans-serif', gap:'15px'}}>
        ...
        <div style={{fontSize:'14px', letterSpacing:'1px', fontWeight:600}}>LOADING DAISY HILL REGISTRATIONS...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background:'#090d16', minHeight:'100vh', color:'#f8fafc', fontFamily:'Inter, system-ui, sans-serif', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ maxWidth:'1300px', margin:'0 auto', display:'flex', flexDirection:'column', gap:'40px' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'20px' }}>
            <div>
              <div style={{fontSize:'11px', color:'#4ade80', fontWeight:'700', letterSpacing:'1px', textTransform:'uppercase', marginBottom:'6px'}}>
                COMMUNITY PORTAL • REGISTRATION SYSTEM
              </div>
              <h1 style={{fontSize:'36px', fontWeight:'800', margin:0, color:'#fff', letterSpacing:'-0.5px'}}>Daisy Hill Farm Registrations</h1>
            </div>
            <div style={{ display:'flex', gap:'15px' }}>
              <div style={{ background:'rgba(17, 24, 39, 0.85)', border:'1px solid #374151', padding:'10px 18px', borderRadius:'8px', display:'flex', flexDirection:'column', gap:'2px' }}>
                <span style={{fontSize:'10px', color:'#94a3b8', fontWeight:'700', textTransform:'uppercase'}}>Total Sectors</span>
                <span style={{fontSize:'13px', fontWeight:'800', color:'#4ade80'}}>{registrations.length} Active</span>
              </div>
              <div style={{ background:'rgba(17, 24, 39, 0.85)', border:'1px solid #374151', padding:'10px 18px', borderRadius:'8px', display:'flex', flexDirection:'column', gap:'2px' }}>
                <span style={{fontSize:'10px', color:'#94a3b8', fontWeight:'700', textTransform:'uppercase'}}>Status</span>
                <span style={{fontSize:'13px', fontWeight:'800', color:'#38bdf8'}}>VERIFIED</span>
              </div>
            </div>
          </div>

          {/* REGISTRATION FORM SECTION */}
          <div style={{ background:'rgba(17, 24, 39, 0.9)', backdropFilter:'blur(10px)', borderRadius:'12px', border:'1px solid #374151', padding:'30px', boxShadow:'0 10px 25px -5px rgba(0,0,0,0.5)' }}>
            <div style={{borderBottom:'1px solid #374151', paddingBottom:'15px', marginBottom:'20px'}}>
              <h2 style={{fontSize:'18px', fontWeight:'800', margin:'0 0 4px 0', color:'#fff'}}>Register New Farm Sector</h2>
              <p style={{fontSize:'12px', color:'#94a3b8', margin:0}}>Add a new property or sector to the Daisy Hill Community Farm network.</p>
            </div>
            ...
            <form onSubmit={handleRegister} style={{display:'flex', flexDirection:'column', gap:'20px'}}>
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:'20px'}}>
                <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                  ...
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. West Valley Plot" 
                    value={farmName}
                ...
                <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                  <label style={{fontSize:'11px', fontWeight:'700', color:'#94a3b8', textTransform:'uppercase'}}>Assigned Sector</label>
                  <select 
                    value={sector}
                ...
                <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                  <label style={{fontSize:'11px', fontWeight:'700', color:'#94a3b8', textTransform:'uppercase'}}>Acreage</label>
                  <input 
                    type="number" 
                    required 
                    placeholder="e.g. 310" 
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                ...
                <input 
                  type="email" 
                  required 
                  placeholder="manager@daisyhill.farm" 
                  value={contactEmail}
              ...
              >
                {submitting ? 'REGISTERING...' : 'SUBMIT REGISTRATION FORM'}
              </button>
            </form>
          </div>
          ...
          <div style={{ background:'rgba(17, 24, 39, 0.85)', backdropFilter:'blur(10px)', borderRadius:'12px', border:'1px solid #374151', padding:'30px', boxShadow:'0 10px 25px -5px rgba(0,0,0,0.5)' }}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid #374151', paddingBottom:'15px', marginBottom:'20px'}}>
              <div>
                <h2 style={{fontSize:'18px', fontWeight:'800', margin:'0 0 4px 0', color:'#fff'}}>Registered Farm Sectors List</h2>
                <p style={{fontSize:'12px', color:'#94a3b8', margin:0}}>Overview of all registered properties and plots under Daisy Hill Community Farm.</p>
              </div>
              <span style={{fontSize:'11px', fontWeight:'700', color:'#4ade80', background:'rgba(74,222,128,0.1)', padding:'5px 12px', borderRadius:'6px', border:'1px solid rgba(74,222,128,0.2)', display:'flex', alignItems:'center', gap:'6px'}}>
                <span style={{width:'6px', height:'6px', background:'#4ade80', borderRadius:'50%', display:'inline-block'}}></span> LIVE REGISTRY
              </span>
            </div>
      ...
      <div style={{ background:'#0b111e', padding:'15px 30px', textAlign:'center', fontSize:'11px', color:'#64748b', borderTop:'1px solid #1f2937', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <span>DAISY HILL COMMUNITY FARM • REGISTRY DASHBOARD</span>
      </div>
    </div>
  );
}