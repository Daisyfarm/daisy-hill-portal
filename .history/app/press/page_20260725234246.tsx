...
export default function PressPage() {
  ...
  const load = async () => {
    ...
    if (!pressData || pressData.length === 0) {
      ...
      if (fallbackPress && fallbackPress.length > 0) {
        setReleases(fallbackPress);
      } else {
        // Fallback default press releases if table is empty
        setReleases([
          {
            id: 1,
            title: 'Judith Plains Harvest Yield Projected Up 15% for Season',
            media_outlet: 'Iron Daisy Radio Network',
            category: 'Press Release',
            content: 'Agricultural analysts across Montana report exceptional kernel fill rates and optimum moisture levels in Judith Plains North. Heavy machinery operators are operating at full capacity to meet storage commitments.',
            author: { username: 'Samuel Founder' },
            created_at: new Date().toISOString()
          },
          {
            id: 2,
            title: 'Live Q&A with Executive Directorate on Infrastructure Expansion',
            media_outlet: 'Montana Agrarian Wire',
            category: 'Interview',
            content: 'Tune in to our evening broadcast at 18:00 MST for an exclusive panel discussion regarding new transport corridors, subsidy structures, and regional zoning permits.',
            author: { username: 'DaisyFarmer' },
  ...
  const publishPress = async (e: any) => {
    e.preventDefault();
    if (!u) {
      alert("Please authenticate your operator profile to publish press releases.");
      return;
    }
  ...
  if (ld) return <div style={{background:'#1a1a1a',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Accessing Press & Media Wire...</div>;
  ...
  return (
    <div style={{ background:'#111', minHeight:'100vh', color:'#fff', fontFamily:'Arial, sans-serif', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ background:'#222', padding:'12px 25px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'2px solid #4a7ab5' }}>
        ...
        <span style={{color:'#fff', fontSize:'11px'}}>OPERATOR BALANCE: ${u?.balance?.toLocaleString() || '0'}</span>
      </div>
                ...
                releases.map(item => (
                  <div key={item.id} style={{ background:'rgba(35,35,35,0.95)', padding:'30px', borderLeft:'6px solid #22c55e', borderRadius:'4px', display:'flex', flexDirection:'column', gap:'12px' }}>
                    ...
                    <div style={{borderTop:'1px solid #444', paddingTop:'10px', marginTop:'5px', display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#888'}}>
                      <span>Author / Spokesperson: <b>{item.author?.username || item.author || 'Corporate Communications'}</b></span>
