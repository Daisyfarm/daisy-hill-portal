...
export default function CompanyPortalPage() {
  ...
  const load = async () => {
    try {
      ...
      if (user) {
        ...
        setU(profile || { id: user.id, username: user.email?.split('@')[0] || 'User', balance: 10000 });

        const { data: comp } = await sb.from('companies').select('*').eq('owner_id', user.id).maybeSingle();
        setMyCompany(comp || null);
      } else {
        setU({ id: 'demo', username: 'Operator', balance: 25000 });
      }
  ...
  const registerCompany = async (e: any) => {
    ...
    await fetch(HK, {
      ...
      body: JSON.stringify({
        content: `🏢 **NEW CORPORATE INCORPORATION**\n**${u.username}** has registered **${form.name}** [${form.tag.toUpperCase()}] within Daisy Hill Community Farm.`
      })
    }).catch(() => {});
