...
export default function NewsPage() {
  ...
  const load = async () => {
    ...
    if (!newsData || newsData.length === 0) {
      ...
      if (fallbackNews && fallbackNews.length > 0) {
        ...
        // Fallback default news bulletins if table is empty
        setNews([
          {
            id: 1,
            title: 'Iron Daisy Agri Network Expansion & Phase II Zoning',
            category: 'Executive Directive',
            content: 'The executive directorate has authorized the immediate opening of Judith Plains North & South for heavy machinery deployment and logistics operations. All operators are advised to review updated route clearances.',
            author: { username: 'Samuel Founder' },
            created_at: new Date().toISOString()
          },
          {
            id: 2,
            title: 'Global Commodity Index: Wheat & Corn Surge',
            category: 'Market Update',
            content: 'Grade A wheat futures have experienced a bullish +4.2% correction following heavy rainfall reports across northern sectors. Operators holding grain storage are advised to monitor index volatility.',
  ...
  const publishNews = async (e: any) => {
    e.preventDefault();
    if (!u) {
      alert("Please authenticate your operator profile to broadcast news.");
      return;
    }
  ...
  if (ld) return <div style={{background:'#1a1a1a',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Accessing News Terminal...</div>;
                ...
                news.map(item => (
                  <div key={item.id} style={{ background:'rgba(35,35,35,0.95)', padding:'30px', borderLeft:'6px solid #4a7ab5', borderRadius:'4px', display:'flex', flexDirection:'column', gap:'12px' }}>
                    ...
                    <div style={{borderTop:'1px solid #444', paddingTop:'10px', marginTop:'5px', display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#888'}}>
                      <span>Author: <b>{item.author?.username || item.author || 'Executive Directorate'}</b></span>
