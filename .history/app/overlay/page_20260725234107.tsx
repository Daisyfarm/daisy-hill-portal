...
export default function OverlayPage() {
  ...
  const load = async () => {
    ...
    if (!overlayData || overlayData.length === 0) {
      ...
      if (fallbackOverlay && fallbackOverlay.length > 0) {
        ...
        // Fallback default overlay widgets if table is empty
        setOverlays([
          {
            id: 1,
            title: 'Montana Harvest Cam HUD',
            overlay_type: 'Full Stream HUD',
            stream_url: 'rtmp://live.network.agri/feed/montana',
            theme: 'Dark Tactical',
            status: 'active',
            creator: { username: 'Samuel Founder' }
          },
          {
            id: 2,
            title: 'Judith Plains Treasury Ticker',
            overlay_type: 'Live Stats Widget',
            stream_url: '',
            theme: 'Emerald Harvest',
            status: 'active',
            creator: { username: 'DaisyFarmer' }
          }
        ]);
      }
    } else {
  ...
  const createOverlay = async (e: any) => {
    e.preventDefault();
    if (!u) {
      alert("Please authenticate your operator profile to deploy stream overlays.");
      return;
    }
  ...
  if (ld) return <div style={{background:'#1a1a1a',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Accessing Overlay Broadcasting Terminal...</div>;
                ...
                overlays.map(ov => (
                  <div key={ov.id} style={{ background:'rgba(35,35,35,0.95)', padding:'25px', borderLeft:'6px solid #4a7ab5', borderRadius:'4px', display:'flex', flexDirection:'column', gap:'12px' }}>
                    ...
                    <div style={{borderTop:'1px solid #444', paddingTop:'10px', marginTop:'5px', display:'flex', justifyContent:'space-between', fontSize:'11px', color:'#888'}}>
                      <span>Creator: <b>{ov.creator?.username || ov.creator || 'Authorized Operative'}</b></span>
                      <span style={{color:'#22c55e', fontWeight:'bold'}}>{ov.status?.toUpperCase() || 'ACTIVE'}</span>
                    </div>
                  </div>
                ))
              )}
